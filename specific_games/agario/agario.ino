...
const char* commandList[NUM_COMMANDS] = {"w"," "};
...
void loop() {
  ...
  // w
  if(digitalRead(switchList[0]) == ON && debounce[0] == 0){
    debounce[0] = 0;
    printString(commandList[0]);
  ...
  // space
  if(digitalRead(switchList[1]) == ON && debounce[1] == 0){
    debounce[1] = 50;
    printString(commandList[1]);
    //printScancode(scancodeArr);
  } else if (digitalRead(switchList[1]) == OFF && debounce[1] > 0){
    debounce[1] -= 1; 
  }
}
